 ...
 *
 * <p>(26.x port) Layers see a render STATE, not the entity, so the worn stack cannot be
 * read here - it is attached each frame in {@code ClientSetup}'s render-state modifier and
 * picked back up through {@link #WORN_PACK}. And drawing a block is a two-step now: a
 * {@link BlockModelResolver} bakes the blockstate into a {@link BlockModelRenderState}, and
 * THAT submits itself to the collector, which draws it later in the frame. The baked state
 * rides the player's render state (a second {@link ContextKey}) exactly as vanilla parks
 * the enderman's carried block on its own - the collector defers, so a layer-local scratch
 * would be a shared mutable across every player in the frame.
 *
 * <p>The geometry below is unchanged from the 1.21.1 original - those numbers were tuned in
 * the pixels and carry forward verbatim.
 *
...
public class WornPackLayer extends RenderLayer<AvatarRenderState, PlayerModel> {
     ...
     */
    public static final ContextKey<ItemStack> WORN_PACK = new ContextKey<>(Packwork.id("worn_pack"));

    /** The baked block model for this player's pack, kept per render state (see class doc). */
    private static final ContextKey<BlockModelRenderState> WORN_PACK_MODEL =
            new ContextKey<>(Packwork.id("worn_pack_model"));
    ...
    public void submit(PoseStack pose, SubmitNodeCollector collector, int packedLight,
        ...
        BlockModelRenderState model = state.getRenderData(WORN_PACK_MODEL);
        if (model == null) {
            model = new BlockModelRenderState();
            state.setRenderData(WORN_PACK_MODEL, model);
        }
