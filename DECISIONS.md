...
- **The 6th tier is DRAGONHIDE** (flag for SapperSquad - veto welcome; alternatives considered:
  Wyrmhide, Drakeskin). *(SUPERSEDED 2026-07-26 — SapperSquad pivoted the tier to the Deep Dark:
...
## 2026-07-26 — pack-first pickup: the Lodestone files what you mine (SapperSquad's ask; reopen with evidence)
...
## 2026-07-26 — the ladder redesigned + tier 6 is SCULKHIDE (SapperSquad's calls; supersedes the ring table below)

- **SapperSquad redesigned the ring materials and pivoted tier 6 away from the dragon.** Locked:
...
- **The name is SCULKHIDE** — keeps the material-word + "-hide" family and points at the tier's
  ...
  strong reason against; **alternates flagged for SapperSquad's veto: Echobound, Wardenhide.** The v1
  "Dragonhide (alternatives: Wyrmhide, Drakeskin)" naming entry below is superseded.
- **Balance, recorded as SapperSquad's design — do not "fix":** tier 6's marginal ring
...
- **Rename mechanics:** registry id `dragonhide_pack` → `sculkhide_pack`, `PackTier.SCULKHIDE`
  ...
  SapperSquad accepts dev-world Dragonhide packs not surviving (noted in CHANGELOG). Found and fixed
...
- **Flipping to Keep captures what's on screen; Tidy Up stays the one-shot re-sort.** Toggling
  ...
  sorted order while keeping the mode — exactly SapperSquad's "Tidy Up still works as a one-shot
...
- **An extension registered is NOT a recipe displayed — the recipe must survive JEI's scan
  first (learned in the field, 2026-07-26).** SapperSquad still saw only info pages. Root cause, read
