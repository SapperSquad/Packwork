...
- **Quill & Ledger v1 files custom tabs by their stamped icon.** *(SUPERSEDED 2026-07-26 —
  Alex played the stamp-gate proxy and couldn't tell what the fitting did, which was the
...
- **Alex's hard constraints, from playtest:** every pack recipe fills ALL NINE cells; the
...
- **Stamp-family matching is ALWAYS-ON for custom tabs — no trinket needed.** Stamp a tab
  with a pickaxe and it gathers tools, out of the box. Alex played the v1 stamp-gate proxy
...
- **CURIOS = the back wear slot. LIVE-verified.** `compat/curios/CuriosCompat` (the one class
  ...
  **Verified** in `runClient -Pcurios`: the player has a back slot, the pack is assigned to it,
  ...
  block-entity, not a Curios slot.~~ **SUPERSEDED 2026-07-26 (Alex's ask): the worn GUI is
