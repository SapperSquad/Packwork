...
- **1.21.5 (items/gametests):** SwordItem/DiggerItem/ArmorItem classes dissolved →
  `PredicateKind` reads components now, tuned to keep Alex's routing (axes carry
...
## Status — 1.0.0 stamped, awaiting Alex's upload

> Newest first. Full source map and roadmap below. Version is **1.0.0** (Alex's call,
> stamped 2026-07-26; jar = `packwork-1.0.0.jar`); the upload itself is Alex's, from
...
**2026-07-26 release stamping — 1.0.0 (Alex's final calls after his confirm pass).**
...
dependency `side="BOTH"`, no `clientSideOnly` flag to contradict it. Gallery = **Alex's 6
...
**2026-07-26 open-from-worn — the Curios-slot GUI gap is CLOSED (Alex's ask).** A pack worn
...
- **Tests: 57 green** with and without `-Pcurios` (3 new gated: `wornOpenBindsAndListsGated`,
  ...
  from the store page; DECISIONS v1 entry superseded. **Wants Alex's eyes in-game:** the
...
7. **Pack-first pickup (Alex's ask: "cobblestone won't automatically go into my backpack").**
...
6. **The ladder redesigned + tier 6 renamed SCULKHIDE (Alex's calls, second ring pass).**
...
5. **The upgrade RING (Alex's playtest calls: nine cells, pack centered, diamonds).** Every
   ...
   iron/DIAMOND (Alex's explicit call); Runed = amethyst/echo shards; Dragonhide = shulker
...
1. **JEI renders the pack ladder as REAL recipes (bug; field-tested twice).** The compat
   ...
   tooltip notes that contents/layout/trinkets/name/stores all carry up. **Alex's field test
